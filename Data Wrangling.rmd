# Data Wrangling
Data wrangling was done outside the shiny app to keep the size of the file small enough to upload to shinyapps.io (http://www.shinyapps.io/) 
...
library(dplyr)
```
My first step was to make a database for each of the years that data can be gathered from, 2008 - current day. It was easier to compute if each databse was one year as it keeps size and thus run times down. The second part of was to scrape the data with the pitchRx packge(https://github.com/cpsievert/pitchRx) and store it in the created database. 
...
```
PitchRx has 5 tables of data that are scraped; atbat, action, pitch, po runner. I was just looking at the atbat and pitch table. (notes on each table: https://github.com/Npennell96/Clayton-Kershaw-Shiny-App/blob/master/Notes/PitchRx%20Variables.rmd). Each table was then pulled from the database with only the wanted column variable, merged and then preseaon games where removed.
...
```
Then just insatnces where Clayton Kershaw pitched where taken and merged.  
...
```
Adding the full name of each pitch type, rather than a two letter abbreviation, and removing the unwated pitch types. As well as adding a simple outcome of a pitch. 

Then was writen to a csv file so as it can be load in the shiny app.
...
```   
To Find the average pitch speed of Kershas main pitches, Four-Seam Fastball, Changeup, Curveball, Slider, from year to year and agian writing to csv.
...
```  
Doing the same but this time for the all players in the league to find the league average for the same four pitches. 
